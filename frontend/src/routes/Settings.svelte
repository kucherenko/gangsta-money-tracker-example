<script lang="ts">
  ...
  async function handleCategorySubmit(e: SubmitEvent) {
    e.preventDefault();
    if (!newName.trim()) { errors = { name: "Name is required" }; return; }
    const data = { name: newName.trim(), color: newColor, type: newType };
    try {
      if (editingId) { await categories.updateCategory(editingId, data); }
      else { await categories.addCategory(data); }
      cancelEdit();
      await categories.load();
    } catch (err: any) { errors = { submit: err.message }; }
  }
  ...
  async function handleExport() {
    try {
      ...
      errors = { import: `Export failed: ${err.message}` };
    }
  }
  ...
  async function handleImportSubmit() {
    ...
    try {
      ...
      errors = { import: `Import failed: ${err.message}` };
    } finally {
  ...
  function activeCryptoCurrencies() {
    return settings.activeCurrencies("crypto");
  }
...
<!-- ─═══ Inlined Tailwind classes, no <style> block needed ═══─ -->
...
<div class="space-y-6 max-w-4xl mx-auto">
  ...
  <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
    ...
    <form onsubmit={handleCategorySubmit} class="mb-6 flex flex-wrap items-end gap-3">
      ...
      <div>
        <input type="color" bind:value={newColor} class="h-9 w-14 rounded-lg border border-gray-300 cursor-pointer" />
      </div>
